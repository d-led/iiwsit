...
/**
 * Build formulas section for detailed math
 */
function buildFormulasSection(): string {
  return `
    <div class="divider mt-8">🤓 For the Nerds: The Math Behind the Scenes</div>
    <div class="collapse collapse-arrow bg-base-200 mt-4">
      <input type="checkbox" />
      <div class="collapse-title text-lg font-medium">Click to view formulas</div>
      <div class="collapse-content">
        <div class="space-y-4 text-sm">
          ${buildTimeBasedFormulas()}
          ${buildMoneyBasedFormulas()}
          ${buildFailureRateFormulas()}
          ${buildDecisionScoringFormulas()}
        </div>
      </div>
    </div>
  `;
...
/**
 * Build time-based formulas
 */
function buildTimeBasedFormulas(): string {
  return `
    <h5 class="font-bold text-base mt-4">Time-Based Calculations:</h5>
    <div class="font-mono text-xs bg-base-300 p-3 rounded overflow-x-auto">
      <p><strong>Total Requests Over Time Horizon:</strong></p>
      <p>Total Requests = Rate (req/hr) × 24 (hr/day) × 365 (days/yr) × Time Horizon (years)</p>
      <p class="mt-2"><strong>Time Saved Per Request:</strong></p>
      <p>Time Saved = Duration (hours) × (Speed Gain % / 100)</p>
      <p class="mt-2"><strong>Total Time Saved:</strong></p>
      <p>Total Time Saved = Time Saved Per Request × Total Requests</p>
      <p class="mt-2"><strong>Total Cost (Time):</strong></p>
      <p>Total Cost = Implementation Time + (Maintenance Time Per Year × Time Horizon)</p>
      <p class="mt-2"><strong>Net Benefit (Time):</strong></p>
      <p>Net Benefit = Total Time Saved - Total Cost</p>
      <p class="mt-2"><strong>ROI (Time):</strong></p>
      <p>ROI = (Net Benefit / Total Cost) × 100%</p>
      <p class="mt-2"><strong>Break-Even (Time):</strong></p>
      <p>Break-Even Years = Total Cost / (Total Time Saved / Time Horizon)</p>
    </div>
  `;
}

/**
 * Build money-based formulas
 */
function buildMoneyBasedFormulas(): string {
  return `
    <h5 class="font-bold text-base mt-4">Money-Based Calculations:</h5>
    <div class="font-mono text-xs bg-base-300 p-3 rounded overflow-x-auto">
      <p><strong>Compute Cost Savings:</strong></p>
      <p>Compute Cost Savings = Total Time Saved × Compute Cost Per Hour</p>
      <p class="mt-2"><strong>Implementation Cost (Money):</strong></p>
      <p>Implementation Cost = Implementation Time × Developer Hourly Rate</p>
      <p class="mt-2"><strong>Maintenance Cost (Money):</strong></p>
      <p>Maintenance Cost = (Maintenance Time Per Year × Time Horizon) × Developer Hourly Rate</p>
      <p class="mt-2"><strong>Total Cost (Money):</strong></p>
      <p>Total Cost = Implementation Cost + Maintenance Cost</p>
      <p class="mt-2"><strong>Net Benefit (Money):</strong></p>
      <p>Net Benefit = Compute Cost Savings - Total Cost</p>
      <p class="mt-2"><strong>ROI (Money):</strong></p>
      <p>ROI = (Net Benefit / Total Cost) × 100%</p>
      <p class="mt-2"><strong>Break-Even (Money):</strong></p>
      <p>Break-Even Years = Total Cost / (Compute Cost Savings / Time Horizon)</p>
    </div>
  `;
}

/**
 * Build failure rate formulas
 */
function buildFailureRateFormulas(): string {
  return `
    <h5 class="font-bold text-base mt-4">Failure Rate Analysis:</h5>
    <div class="font-mono text-xs bg-base-300 p-3 rounded overflow-x-auto">
      <p><strong>Current Failed Requests:</strong></p>
      <p>Current Failed = Total Requests × (Current Failure Rate % / 100)</p>
      <p class="mt-2"><strong>Bug-Induced Failed Requests:</strong></p>
      <p>Bug Failed = Total Requests × (Bug Failure Rate % / 100)</p>
      <p class="mt-2"><strong>Net Failure Change:</strong></p>
      <p>Net Change = Current Failed - Bug Failed</p>
      <p class="mt-2"><strong>Failure Rate Change:</strong></p>
      <p>Rate Change % = Current Failure Rate % - Bug Failure Rate %</p>
      <p class="text-xs text-base-content/70 italic mt-2">(Positive = improvement, Negative = degradation)</p>
    </div>
  `;
}

/**
 * Build decision scoring formulas
 */
function buildDecisionScoringFormulas(): string {
  return `
    <h5 class="font-bold text-base mt-4">Decision Confidence Scoring:</h5>
    <div class="font-mono text-xs bg-base-300 p-3 rounded overflow-x-auto">
      <p><strong>Weight Distribution:</strong></p>
      <p>Cost Weight = (100 - Optimization Preference) / 100</p>
      <p>Throughput Weight = Optimization Preference / 100</p>
      <p class="mt-2"><strong>Score Components (max 100 points):</strong></p>
      <ul class="list-disc list-inside ml-2 mt-1">
        <li>Financial Benefit: up to 40 × Cost Weight points</li>
        <li>Time Benefit: up to 40 × Throughput Weight points</li>
        <li>Financial ROI: up to 30 × Cost Weight points</li>
        <li>Time ROI: up to 30 × Throughput Weight points</li>
        <li>Financial Break-Even: up to 20 × Cost Weight points</li>
        <li>Time Break-Even: up to 20 × Throughput Weight points</li>
        <li>Failure Rate Impact: up to 15 points</li>
        <li>Speed Gain Magnitude: up to 10 points</li>
      </ul>
      <p class="mt-2"><strong>Confidence Calculation:</strong></p>
      <p>Confidence % = (Total Score / Max Score) × 100</p>
      <p class="mt-2"><strong>Decision Thresholds:</strong></p>
      <p>≥ 60% = YES, 40-59% = MAYBE, < 40% = NO</p>
    </div>
  `;
}
